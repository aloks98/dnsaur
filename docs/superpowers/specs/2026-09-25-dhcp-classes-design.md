...
- **Pool**: a `start`–`end` range inside a scope's subnet, with an optional `class`. A pool with a class serves only clients in that class. A pool without one serves anyone **except** members of a class that owns a pool in the same scope: Kea's allocator otherwise hands a member the first pool it may use, so the renderer guards each open pool with a generated class `dnsaur-open-<scope id>` (`not member('iot') and …`). A member whose class pool is full gets no address in that scope; that is the price of "lands in 10.0.20.x" meaning what it says. A class with no pool in a scope draws from the open pools like anyone.
...
- **`subnet4[].pools`**: one entry per pool in position order: `{"pool": "<start> - <end>"}`, plus the pool's class (`client-class` below Kea 3.0, `client-classes: [name]` from 3.0) when it has one, plus that class's option-data **again at the pool level** (PXE fields have no pool form; the class level carries them). A pool with no class in a scope where some class owns a pool gets the scope's guard class instead. Class names starting `dnsaur-` and Kea's own ALL, KNOWN, UNKNOWN, BOOTING, DROP are refused for user classes. Kea's option precedence is host reservation > pool > subnet > shared network > class > global, so a class's options only beat the scope's own when they sit on the pool; class-level option-data alone loses to any value the scope sets (DNS servers, which every scope renders, included).
