...
| HA | Instance-to-instance config sync, primary/replica, each instance self-contained on its own SQLite. Shared-Postgres and multi-master CRDT models rejected (DB host becomes a SPOF; conflict-resolution complexity not worth it). **Superseded (2026-09-11)** by `2026-09-11-config-sync-design.md`, which is what shipped: the choice above stands, but the Phase 1.5 mechanism sketched below — join token, self-signed certificate pinned by fingerprint, long-poll push channel, replica UIs proxying writes to the primary, federated stats — was dropped for a polled config bundle, AXFR for zone data, and a replica that refuses synced writes outright. |
