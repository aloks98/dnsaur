...
--
-- BIGINT here (BIGSERIAL for id) -- see 0010/0011 for why: postgres INTEGER
-- is 32-bit and signed, and a serial is a uint32 that does not fit its
-- positive range at the top of the space, which is exactly the region
-- SerialNewer exists for; a unix millisecond timestamp overflows it too.
CREATE TABLE zone_notifies (
  id              BIGSERIAL PRIMARY KEY,
  zone_id         BIGINT  NOT NULL REFERENCES zones(id) ON DELETE CASCADE,
  target          TEXT    NOT NULL,
  pending_serial  BIGINT  NOT NULL DEFAULT 0,
  notified_serial BIGINT  NOT NULL DEFAULT 0,
