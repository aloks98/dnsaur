...
func TestLocalSettingKey(t *testing.T) {
	for _, key := range []string{
		"instance.id", "serve.dot.listen", "serve.tls.cert",
		// sync.tsig_key_id among them, and it is the one written with a
		// bumping Set: the version it moves is what fetches the bundle, and
		// the row itself still stays on the main that designated it.
		"sync.peer_url", "sync.token", "sync.tsig_key_id", StatsWatermarkKey,
	} {
...
func TestBundleCarriesClassesAndPools(t *testing.T) {
	forEachDriverPair(t, func(t *testing.T, main, replica Store) {
		...
		// class or a scope under this id from an earlier test. Everything
		// else is the main's.
		classAsReplicaHolds := func(got Class) Class {
			w := wantClass
		...
		if got, err := replica.DHCP().Class(ctx, cid); err != nil || !reflect.DeepEqual(got, classAsReplicaHolds(got)) {
