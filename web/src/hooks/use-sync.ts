...
import type { PairingCode, SyncStatus } from "../api/types";
...
/**
 * POST /sync/pairing-code — "Add replica" on a main.
 *
 * Nothing to invalidate: minting a code moves no read this app makes, and
 * the registry entry it eventually buys appears when the other box pairs,
 * which is its own poll away.
 *
 * The answer is held by the mutation itself rather than copied into state.
 * There is no read endpoint — only the hash is stored, and minting a second
 * code voids the first — so the mutation's own `data` *is* the code's
 * lifetime: `reset()` dismisses it, and a second press replaces it.
 */
export function usePairingCode() {
  return useMutation({ mutationFn: () => api.post<PairingCode>("/sync/pairing-code") });
}

/**
 * POST /sync/follow — the other end of the pairing, on the box that is to
 * become a replica: it spends the code against the main, writes
 * `sync.peer_url` and the secret into `sync.token` in one settings write,
 * and pulls once.
 *
 * Three reads move at once because the server wrote settings and changed
 * this box's role: the settings themselves, the sync status this band
 * renders, and the resolver status every other screen reads before deciding
 * whether its write controls are live (useManagedBy).
 */
export function useFollow() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (v: { peer_url: string; code: string }) => api.post<void>("/sync/follow", v),
    onSuccess: () => {
